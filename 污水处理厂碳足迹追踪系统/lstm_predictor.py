...
class DataSimulator:
    ...
    def _create_monthly_data(self, daily_df):
        ...
        # 按月聚合 - 统一使用日均值乘以30的标准化处理
        monthly_df = df.resample('M').agg({
            '处理水量(m³)': 'mean',  # 改为日均值，保持一致性
            '电耗(kWh)': 'mean',  # 改为日均值
            'PAC投加量(kg)': 'mean',  # 改为日均值
            'PAM投加量(kg)': 'mean',  # 改为日均值
            '次氯酸钠投加量(kg)': 'mean',  # 改为日均值
            '进水COD(mg/L)': 'mean',  # 平均浓度
            '出水COD(mg/L)': 'mean',  # 平均浓度
            '进水TN(mg/L)': 'mean',  # 平均浓度
            '出水TN(mg/L)': 'mean',  # 平均浓度
            'total_CO2eq': 'mean',  # 关键修改：使用日均值保持量级一致
            '自来水(m³/d)': 'mean',
            '脱水污泥外运量(80%)': 'mean'
        }).reset_index()

        # 标准化为月度表示（乘以30天）- 保持科学性和一致性
        scaling_columns = [
            '处理水量(m³)', '电耗(kWh)', 'PAC投加量(kg)', 'PAM投加量(kg)',
            '次氯酸钠投加量(kg)', 'total_CO2eq', '自来水(m³/d)', '脱水污泥外运量(80%)'
        ]
